...
class Trainer(object):
    def __init__(self, args):
        ...
        optimizer = torch.optim.Adam(train_params, args.lr, amsgrad=True, eps=1e-4)

        # Define Criterion
        # whether to use class balanced weights
        if args.use_weighted_loss:
            class_w_extra_weights = 3
            weight = (1./(self.nclass+3*class_w_extra_weights)) * np.ones(self.n_class, dtype=np.float32)
            weight[-1] *= 4
            weight[-2] *= 4
            weight[-3] *= 4
        ...
        if args.cuda:
            ...
            self.model.decoder.last_conv = nn.Sequential(*decoder_modules, nn.Conv2d(num_ftrs, self.nclass, 1))
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(t_bar):
            t_start = time.time()
            with torch. no_grad():
            ...
            t_bar.set_description('Train loss: %.3f' % (train_loss / (i + 1)))
            self.writer.add_scalar('train/total_loss_iter', loss.item(), i + num_img_tr * epoch)
            # t_logging = time.time()
            # print("data {}".format(t_data - t_start))
            # print("model {}".format(t_model - t_data))
            # print("backward {}".format(t_backward - t_model))
            # print("logging {}".format(t_logging - t_backward))
    ...
    def validation(self, epoch):
        ...
        for i, sample in enumerate(t_bar):
            ...
            loss = self.criterion(output, target)
            test_loss += loss.item()
            t_bar.set_description('Test loss: %.3f' % (test_loss / (i + 1)))
        ...
        per_class_accuracies = self.evaluator.class_accuracies()
        ...
        for name, c in zip(self.class_names, per_class_accuracies):
            self.writer.add_scalar('val/{}'.format(name), c, epoch)
...
def main():
    ...
    t1 = time.time()
    print("Setting up took {} seconds...".format(t1-t0))
    for epoch in range(trainer.args.start_epoch, trainer.args.epochs):
        # ts = time.time()
        trainer.training(epoch)
        # tte = time.time()
        # print("Epoch Done {}...".format(tte-ts))
        if not trainer.args.no_val and epoch % args.eval_interval == (args.eval_interval - 1):
            trainer.validation(epoch)
            # tve = time.time()
            # print("Validation Done {}...".format(tve-tte))
